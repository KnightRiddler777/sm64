...
s32 update_behind_mario_camera(struct Camera *c, Vec3f focus, Vec3f pos) {
    ...
    s16 pitch;
    s16 yaw;
    s16 goalPitch = gMarioObject->oFaceAnglePitch;
    ...
    f32 maxDist = 800.f;
    f32 focYOff = 125.f;
    if (goalPitch > 0x3E00) goalPitch = 0x3E00;
...
BAD_RETURN(s32) cutscene_prepare_cannon_start(struct Camera *c) {
    ...
    mtxf_mul_vec3f(gWorldToLocalGravTransformMtx, sCutsceneVars[3].point);
...
BAD_RETURN(s32) cutscene_enter_painting(struct Camera *c) {
	...
	mtxf_mul_vec3f(gWorldToLocalGravTransformMtx, paintingPos);
...
BAD_RETURN(s32) cutscene_exit_painting_start(struct Camera *c) {
    ...
    vec3f_set(sCutsceneVars[2].point, 258.f, 300.f, 1189.f);
...
BAD_RETURN(s32) cutscene_enter_cannon_raise(struct Camera *c) {
    ...
    if ((o = sMarioCamState->usedObj) != NULL) {
        ...
        c->focus[1] = o->oPosY;
        c->focus[2] = o->oPosZ;
	mtxf_mul_vec3f(gWorldToLocalGravTransformMtx, c->focus);
	sCutsceneVars[0].point[1] = c->focus[1];
        cannonFocus[0] = 0.f;
        cannonFocus[1] = 100.f;
        cannonFocus[2] = 0.f;
...
BAD_RETURN(s32) cutscene_enter_cannon_start(struct Camera *c) {
	...
	mtxf_mul_vec3f(gWorldToLocalGravTransformMtx, sCutsceneVars[0].point);
