...
s32 set_pole_position(struct MarioState *m, f32 offsetY) {
    ...
    m->pos[0] = objPos[0];
    m->pos[2] = objPos[2];
    m->pos[1] = objPos[1] + marioObj->oMarioPolePos + offsetY;
    ...
    if (m->pos[1] > ceilHeight - 150.0f) {
        m->pos[1] = ceilHeight - 150.0f;
        marioObj->oMarioPolePos = m->pos[1] - objPos[1];
    }
    ...
    if (m->pos[1] < floorHeight) {
        ...
        m->pos[1] = objPos[1] - m->usedObj->hitboxDownOffset;
        set_mario_action(m, ACT_FREEFALL, 0);
...
s32 perform_hanging_step(struct MarioState *m, Vec3f nextPos) {
    ...
    ceilOffset = ceilHeight - (nextPos[1] + 150.0f);
    if (ceilOffset < -30.0f) {
        return HANG_HIT_CEIL_OR_OOB;
    }
    if (ceilOffset > 30.0f) {
...
s32 update_hang_moving(struct MarioState *m) {
    ...
    if (!m->ceil) return HANG_LEFT_CEIL;
...
s32 act_start_hanging(struct MarioState *m) {
    ...
    if (!m->ceil) return set_mario_action(m, ACT_FREEFALL, 0);
...
s32 act_ledge_grab(struct MarioState *m) {
    ...
    s32 hasSpaceForMario = (m->ceilHeight - m->floorHeight >= 150.0f);
...
s32 act_grabbed(struct MarioState *m) {
	...
	vec3f_sub(m->pos, &m->marioObj->oPosX);
#if ENABLE_RUMBLE
...
s32 act_tornado_twirling(struct MarioState *m) {
    ...
    f32 dx = (gMarioObject->oPosX - usedObj->oPosX) * 0.95f;
    f32 dz = (gMarioObject->oPosZ - usedObj->oPosZ) * 0.95f;
    ...
    mtxf_mul_vec3f(gWorldToLocalGravTransformMtx, nextPos);
